...
import {getCaregivers, DeleteUser} from "../../../data/userData";
...
function CaregiverPage() {
    ...
    useEffect(async () => {
        let data = await getCaregivers(token);
        if(data != null){
            setUsers(data)
    ...
    const handleEdit = async (userId) => {
        history.push(`/caregivers/${userId}`, {params: userId});
    }
                        ...
                        <TableBody>
                            {users && Object.values(users.users).slice((page - 1) * itemsPerPage, page * itemsPerPage).map(user => (
                                <TableRow key={user._id}>
                                    ...
                                    <TableCell align="left">
                                        <IconButton aria-label="edit user" disabled={user.roles[0].name != "caregiver"} onClick={() => handleEdit(user._id)}>
                                            <CreateIcon fontSize="small"/>
                                        </IconButton>
                                        <IconButton aria-label="delete user" disabled={user.roles[0].name != "caregiver"} onClick={() => handleDelete(user._id)}>
